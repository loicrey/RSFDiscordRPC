...
That script builds `Release|Win32`, creates `dist/RSFDiscordRPC-<version>/`, and creates `dist/RSFDiscordRPC.zip`.
The zip contains `RSFDiscordRPC.dll` and `RSFDiscordRPC/RSFDiscordRPC.ini` at the archive root.

Optional environment overrides:

- `RSF_MSBUILD_PATH`: override the `MSBuild.exe` path used by the build script
- `RELEASE_VERSION`: override the package version used in `dist/RSFDiscordRPC-<version>`
- `RSF_VERSION`: legacy fallback for the package version override
